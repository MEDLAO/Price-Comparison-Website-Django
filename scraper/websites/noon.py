...
async def fetch_noon(s, url):
    ...
    while data is None:
        try:
            async with s.get(f"https://www.noon.com/egypt-en/search/?limit=50&originalQuery=smart%20watch&page={url}&q=smart%20watch&searchDebug=false&sort%5Bby%5D=popularity&sort%5Bdir%5D=desc", headers=headers) as r:
                r.raise_for_status()
                data = await r.text()
                print(r.status)
